    ...
    BarChartCanvasScaffold(
        ...
        content = { canvasHeight, gap, barWidth ->
            displayData.fastForEachIndexed { index, barData ->
                ...
                if (isClickInsideBar(clickedOffSet, rectTopLeft, rectSize)) {
                ...
                val textOffsetY = if (barData.yValue < 0) {
                    rectTopLeft.y - textLayoutResult.size.height - 5
                } else {
                    rectTopLeft.y + rectSize.height + 5
                }
                ...
                if (labelConfig.showLabel) {
                    drawText(
                        ...
                        topLeft = Offset(
                            rectTopLeft.x + barWidth / 2 - textLayoutResult.size.width / 2,
                            textOffsetY
                        )
                    )
                }
...
fun BarChart(
    ...
    BarChartCanvasScaffold(
        ...
        displayData.fastForEachIndexed { index, barData ->
            ...
            val cornerRadius = if (barChartConfig.showCurvedBar) {
                CornerRadius(x = barWidth / 4, y = barWidth / 4)
            } else {
                CornerRadius.Zero
            }
            val textLayoutResult = textMeasurer.measure(
...
private fun DrawScope.backgroundColorBar(
    ...
    drawRoundRect(
        ...
        size = Size(
            width = barWidth,
            height = if (canDrawNegativeChart) maxHeight.absoluteValue.div(2) else maxHeight.absoluteValue
        ),
        cornerRadius = cornerRadius
    )
}
...
private fun getDisplayData(
    ...
    return if (data.size < minimumBarCount) {
        // Add empty bar
...
private fun isClickInsideBar(clickOffset: Offset, rectTopLeft: Offset, rectSize: Size): Boolean {
    return clickOffset.x in rectTopLeft.x..(rectTopLeft.x + rectSize.width) &&
 ...
 */
@Composable
private fun BarChartCanvasScaffold(
    modifier: Modifier = Modifier,
    ...
    Canvas(
        modifier = modifier
            .then(
                if (showAxisLines) Modifier.drawAxisLine(
                    hasNegativeValues = canDrawNegativeChart,
                    axisLineColor = axisLineColor
                ) else Modifier
            )
            .then(
                if (showRangeLines) Modifier.drawRangeLines(
                    hasNegativeValues = canDrawNegativeChart,
